...
export default function Catalog() {
  ...
  const categories = [
    "Todos",
    ...Array.from(new Set(products.map((p) => p.category?.name ?? "Otros"))),
  ];
  ...
  const handleAdd = (p: Product) => {
    ...
    setTimeout(
      () => setRecentlyAdded((prev) => prev.filter((id) => id !== p.id)),
      1200,
    );
  };

  const quantityInCart = (id: string) =>
      ...
      <div className="catalogo-container">
        ...
        {loading ? (
          ...
          <div className="catalogo-vacio">
            ...
            <button
              className="cat-btn cat-btn-activa"
          ...
          <div className="catalogo-grid">
            {filtered.map((prod) => {
              ...
              const imageUrl =
                getImageUrl(prod) ?? productImagesByName[prod.name] ?? null;

              return (
                <div key={prod.id} className="prod-card">
                  <span className="prod-cat-badge">
                    {prod.category?.name ?? "Otros"}
                  </span>
                  {imageUrl ? (
                    <img
                  ...
                  {inCart > 0 && (
                    <p className="prod-en-carrito">✓ {inCart} en carrito</p>
                  )}
